...
import { X, ChevronUp, ChevronDown, Wind, Snowflake, Droplets, RefreshCcw, Zap, Power, Bell } from "lucide-react";
...
const AcDetailModal = ({ device, isOpen, onClose }: AcDetailModalProps) => {
    ...
    const BINARY_SWITCHES = [
        ...
        { key: "screen", name: "Display", icon: Snowflake }, // Using snowflake as placeholder, but will map correctly
        ...
        <AnimatePresence>
            <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
                ...
                <motion.div
                    ...
                    className="relative w-full max-w-lg glass-dark rounded-[2.5rem] p-8 text-white shadow-2xl overflow-hidden"
                >
                    <button
                        onClick={onClose}
                    ...
                    <div className="text-center mb-6">
                        <h2 className="text-2xl font-bold">{device.nickName}</h2>
                        <div className="flex items-center justify-center gap-2 mt-1">
                            <span className={cn("w-2 h-2 rounded-full", device.isOnline === "1" ? "bg-green-500" : "bg-red-500")} />
                            <p className="text-white/60 text-sm">Indoor {state?.currentTemperature}°C</p>
                        </div>
                    </div>
                    ...
                    <div className="overflow-y-auto max-h-[70vh] custom-scrollbar pr-2 -mr-2">
                        ...
                        <div className="flex flex-col items-center gap-6 mb-8 mt-4">
                            <div className="relative w-56 h-56 flex items-center justify-center">
                                ...
                                <div className="absolute inset-4 rounded-full border-4 border-primary/20 border-dashed animate-[spin_20s_linear_infinite]" />
                                
                                <div className="flex flex-col items-center gap-1 z-10">
                                    <button 
                                        onClick={() => handleUpdate({ targetTemperature: (state?.targetTemperature ?? 25) + 1 })}
                                        className="p-2.5 bg-white/10 rounded-2xl hover:bg-white/20 active:scale-95 transition-all"
                                    >
                                        <ChevronUp size={28} />
                                    </button>
                                    
                                    <div className="text-7xl font-bold tracking-tighter tabular-nums my-1">
                                        {state?.targetTemperature}
                                        <span className="text-2xl font-medium absolute mt-2 ml-1 text-white/40">°C</span>
                                    </div>
                        ...
                        <div className="flex justify-center mb-8">
                            ...
                            >
                                ...
                                <span className="text-lg">{state?.powerSwitch === 1 ? "System Off" : "System On"}</span>
                            </button>
                        </div>
                        ...
                        <div className="space-y-6 pb-4">
                            {/* Modes */}
                            <div className="bg-white/5 p-5 rounded-[2rem]">
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                <div className="bg-white/5 p-5 rounded-[2rem]">
                                    ...
                                    <div className="grid grid-cols-2 gap-2">
                                        {FAN_SPEEDS.slice(0, 4).map((speed) => (
                                            <button
                                                key={speed.id}
                                ...
                                <div className="bg-white/5 p-5 rounded-[2rem] flex flex-col">
                                    ...
                                    <div className="flex-1 flex flex-col gap-2">
                                        <button
                                            ...
                                            className={cn(
                                                ...
                                                state?.verticalDirection === 8 ? "bg-blue-500 text-white" : "bg-white/5 text-white/30"
                                            )}
                                        >
                                        ...
                                        <button
                                            ...
                                            className={cn(
                                                "flex-1 rounded-xl text-xs font-bold transition-all flex items-center justify-center gap-2",
                                                state?.horizontalDirection === 8 ? "bg-blue-500 text-white" : "bg-white/5 text-white/30"
                                            )}
                                        >
                                        ...
                                        {GEN_MODES.map((mode) => (
                                            <button
                                                ...
                                                className={cn(
                                                    ...
                                                    state?.generatorMode === mode.id ? "bg-amber-500 text-white" : "bg-white/5 text-white/30"
                                                )}
                                            >
                                        ...
                                        {BINARY_SWITCHES.map((sw) => (
                                            <button
                                                ...
                                                className={cn(
                                                    ...
                                                    state?.[sw.key as keyof typeof state] === 1 ? "bg-primary/20 text-primary" : "bg-white/5 text-white/20"
                                                )}
                                            >
                                                <sw.icon size={16} />
                                                <span className="text-[10px] font-bold truncate w-full">{sw.name}</span>
                                            </button>
                                        ))}
                                    </div>
                                </div>
